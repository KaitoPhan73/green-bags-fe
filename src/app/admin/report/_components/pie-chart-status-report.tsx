...
import { chartStatusConfig } from "./config";
import { TPieChart } from "@/schema/order.schema";
import Image from "next/image";

type Props = {
  data: TPieChart[];
};
export default function PieChartStatusReport({ data }: Props) {
  ...
  const chartData = data.map((item) => ({
    label: chartStatusConfig[item.status].label,
    value: item.count,
    fill: chartStatusConfig[item.status].color,
  }));
